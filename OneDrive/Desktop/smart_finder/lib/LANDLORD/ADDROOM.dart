...
class _HotspotEditorState extends State<HotspotEditor> {
  ...
  Future<void> _handlePanoTap(
    ...
    setState(() {
      final list = hotspotsByImageIndex[currentIndex] ?? <Hotspot>[];
      list.add(newHotspot);
      hotspotsByImageIndex[currentIndex] = list;
      addMode = false;
    });
  }
  ...
  void _showHotspotActions(Hotspot h, int idx) async {
    ...
    if (action == 'jump') {
      ...
      if (newTarget != null) {
        setState(() {
          final list = _hotspotsForCurrent();
          list[idx] = h.copyWith(targetImageIndex: newTarget);
          hotspotsByImageIndex[currentIndex] = List.from(list);
        });
      }
    } else if (action == 'label') {
      final newLabel = await _askLabel();
      setState(() {
        final list = _hotspotsForCurrent();
        list[idx] = h.copyWith(label: newLabel);
        hotspotsByImageIndex[currentIndex] = List.from(list);
      });
    }
  }
                  ...
                  child: ClipRRect(
                    ...
                    child: DecoratedBox(
                      ...
                      child: pano.Panorama(
                        ...
                        onTap: _handlePanoTap, // radians
                        hotspots: [
                          for (final entry
                              in _hotspotsForCurrent().asMap().entries)
                            pano.Hotspot(
                              longitude: entry.value.dx,
                              latitude: entry.value.dy,
                              width: 64,
                              height: 64,
                              widget: GestureDetector(
                                onTap: () =>
                                    _showHotspotActions(entry.value, entry.key),
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: const Icon(Icons.place, size: 22, color: Colors.redAccent),
        ),
      ],
    );
  }
