...
import 'package:smart_finder/TENANT/TGMAP.dart'; // keep if used elsewhere
...
class _TenantApartmentState extends State<TenantApartment> {
  ...
  /// Fetch all rooms and filter for approved + optional landlord **client-side**
  /// (works on older supabase_flutter where `.or/.filter` aren’t available).
  Future<void> _fetchRooms() async {
    ...
    try {
      ...
      // NOTE: No `.or()` / `.filter()` here to avoid SDK-compat errors.
  ...
  void _subscribeApprovedRooms() {
    ...
    _roomsChannel = _sb.channel('tenant-approved-rooms')
      ...
      // Updates (status becomes approved/published or gets unapproved)
      ..onPostgresChanges(
